...
class SchedulerService:
    ...
    async def start(self):
        ...
        try:
            ...
            try:
                # 定时爬取任务：德国时间下午3点 / Scheduled crawling task: 3 PM German time
                self.scheduler.add_job(
                    self._scheduled_crawl_job,
                    CronTrigger(hour=15, minute=30, timezone=self.german_tz),  # 德国时间下午3.30点 / 3.30 PM German time
                    id="daily_crawl",
                    name="Daily Job Crawling",
                    replace_existing=True
                )
                logger.info("✅ 定时爬取任务已设置：德国时间每天下午3点 / Scheduled crawling task set: 3 PM German time daily")
                
                # 数据清理任务：德国时间凌晨2点 / Data cleanup task: 2 AM German time
                self.scheduler.add_job(
                    self._scheduled_cleanup_job,
                    CronTrigger(hour=2, minute=0, timezone=self.german_tz),  # 德国时间凌晨2点 / 2 AM German time
                    id="daily_cleanup",
                    name="Daily Data Cleanup",
                    replace_existing=True
                )
                logger.info("✅ 数据清理任务已设置：德国时间每天凌晨2点 / Data cleanup task set: 2 AM German time daily")
            ...
            logger.info(f"Next crawl scheduled for: German time 15:00 daily")
            logger.info(f"Next cleanup scheduled for: German time 02:00 daily")
    ...
    async def _scheduled_crawl_job(self):
        ...
        按照README要求：每日下午2点定时爬取预设岗位数据 / According to README: daily crawling at 2 PM for preset job titles
    ...
    async def _scheduled_cleanup_job(self):
        ...
        按照README要求：每天凌晨2点访问Chroma中所有岗位URL，如果URL失效则清理
        清理14天之前的岗位数据，防止僵尸岗位
        """
        try:
            ...
            # 检查URL有效性和清理过期数据 / Check URL validity and cleanup expired data
            cleanup_count = 0
            ...
            for i, job_id in enumerate(all_jobs_data['ids']):
                try:
                    ...
                    should_cleanup = await self._should_cleanup_job(job_url, created_at)
    ...
    async def _should_cleanup_job(self, job_url: str, created_at: str) -> bool:
        """
        检查职位是否需要清理 / Check if job should be cleaned up
        ...
        Returns:
            是否需要清理 / Whether cleanup is needed
        """
        try:
            ...
            if created_at:
                try:
                    ...
                    if datetime.now() - job_date > timedelta(days=14):
                        logger.debug(f"Job older than 14 days, marking for cleanup: {job_url}")
                        return True
                except Exception as e:
                    logger.warning(f"Failed to parse job date {created_at}: {e}")
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                    async with session.head(job_url) as response:
                        if response.status >= 400:
                            logger.debug(f"Invalid URL (status {response.status}), marking for cleanup: {job_url}")
                            return True
                return False
            except Exception as e:
                logger.debug(f"URL check failed, marking for cleanup: {job_url} - {e}")
                return True
